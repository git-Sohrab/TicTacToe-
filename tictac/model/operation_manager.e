...
feature -- commands
	...
	do
		check attached undo_list.item as cmd then						-- check to see if item to undo is not void
			...
			if undo_list.count > 0 then undo_list.remove end			-- cannot remove from undo list if its empty
		end
	end
	...
	do
		check attached redo_list.item as cmd then						-- check to see if item to redo is not void
			...
			if redo_list.count > 0 then redo_list.remove end			-- cannot remove from redo list if its empty
		end
	end
